...
# Revert
def run_eval_after_each_commit(branch: str, backend: str, commit0_config_file: str) -> str:
    eval_cmd = f"python -m commit0 evaluate --branch {branch} --backend {backend} --commit0-config-file {commit0_config_file} --timeout 100"
    try:
        result = subprocess.run(
            eval_cmd, shell=True, capture_output=True, text=True, check=True
        ...
        print(f"Error running eval command: {e}")
        # Return stderr or a default empty JSON array as fallback
...
def run_agent_for_repo(
    ...
    print(f"Initial Evaluation Results: '{initial_eval_results}'")  # Debugging output
    if initial_eval_results:
        try:
            initial_eval_results_list = json.loads(initial_eval_results)
        except json.JSONDecodeError as e:
            print("Failed to decode JSON:", e)  # Debugging output for malformed JSON
            initial_eval_results_list = []  # Fallback to an empty list if decoding fails
    else:
        print("No data in initial_eval_results.")  # Debugging output for empty result
        initial_eval_results_list = []  # Set default if initial_eval_results is empty
    # initial_eval_results_list = json.loads(initial_eval_results)
    ...
    for function_batch in function_batches:
        ...
        while retries < agent_config.max_retries and not success:
            ...
            print(f"Current Evaluation Results: '{current_eval_results}'", flush=True)
            if current_eval_results:
                try:
                    current_eval_results_list = json.loads(current_eval_results)
                except json.JSONDecodeError as e:
                    print("Failed to decode current JSON:", e, flush=True)  # Debugging output for malformed JSON
                    current_eval_results_list = []  # Fallback to an empty list if decoding fails
            else:
                print("No data in current_eval_results.", flush=True)  # Debugging output for empty result
                current_eval_results_list = []
            # current_eval_results_list = json.loads(current_eval_results)
