...
def extract_functions_to_implement(source_files: List[Path]) -> Dict[str, Set[str]]:
    """
    ...
    Returns:
        Dict[str, str]: Mapping of file paths to function/method names that need implementation.
    """
    functions_to_implement = defaultdict(set)
        ...
        for node in ast.walk(tree):
            if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
                ...
                if needs_implementation:
                    functions_to_implement[file_path.name].add(node.name)
    ...
    for test_file in test_files:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.FunctionDef) and node.name.startswith("test_"):
                identifier = f"tests/{test_file.name}::{node.name}"
                test_cases[identifier] = test_file
            elif isinstance(node, ast.ClassDef) and node.name.startswith("Test"):
                for child in node.body:
                    if isinstance(child, ast.FunctionDef) and child.name.startswith("test_"):
                        identifier = f"tests/{test_file.name}::{node.name}::{child.name}"
                        test_cases[identifier] = test_file
    return test_cases


def map_tests_to_functions(test_cases: Dict[str, Path], functions_to_implement: Dict[str, Set[str]]) -> Dict[str, Dict[str, Set[str]]]:
    """
    ...
    Args:
        ...
        functions_to_implement (Dict[str, str]): Set of function/method names that need implementation.

    Returns:
        Dict[str, Dict[str, Set[str]]]: Dictionary mapping test case identifiers to a dictionary that maps from the source file to a set of functions involved.
    """
    mapping = {}
    ...
    for test_id, test_file in test_cases.items():
        ...
        if len(parts) == 2:
            ...
            if target_node:
                ...
                mapping[test_id] = called_functions
        elif len(parts) == 3:
...
def get_called_functions(test_node: ast.FunctionDef, functions_to_implement: Dict[str, Set[str]]) -> Dict[str, Set[str]]:
    """
    ...
    Returns:
        Dict[str, Set[str]]: Mapping from source file to function names that are called within the test function.
    """
    ...
    for node in ast.walk(test_node):
        if isinstance(node, ast.Call):
            ...
            if isinstance(node.func, ast.Name):
                ...
                for file_name, functions in functions_to_implement.items():
                    if func_name in functions:
                        called_functions[file_name].add(func_name)
...
def get_test_mapping(repo_path: Path,) -> Dict[str, Dict[str, Set[str]]]:
    """
    Function which returns mapping of tests to their files & a set of the related functions, given path to the repository.
    """
